...
function request(requestConfig){
    ...
    delete  requestConfig.params.session_id;
    if(getApp() != undefined ){
        requestConfig.params.qxinli_uid = getApp().globalData.uid;
       // requestConfig.params.qxinli_uid = 248;
    }
    ...
    wx.request({
        ...
        success: function(res) {
            console.log('resURL:'+wholeUrl+'data:'+body);
            console.log(res);
...
function requestForComments(that,pageIndex,action){
    ...
    config.callback.onSuccess=function(data){
        console.log("DDD------");
        that.data.currentAction = request_none;
        log("data.infos.length:"+data.infos.length);
       // hideLoadingDialog(that);
        var currentDatas = that.getListFromNetData(data);//todo 如果是其中一个字段,需要取出.
...
function isNetAvaliable(success,fail){
    wx.getNetworkType({
        success: function(res) {
            var networkType = res.networkType; // 返回网络类型2g，3g，4g，wifi
            if(networkType == '2g' || networkType == '3g' || networkType == '4g' ||networkType == 'wifi' ){
                success();
            }else {
                fail();
            }
...
function reLogin(){
    ...
    isNetAvaliable(function(){
        var count = getApp().globalData.loginRetryCount;
        if(count <5){
            login();
        }
    },function(){
...
function login(){
    ...
    //调用登录接口
    if(getApp().globalData.isLogining){
        return;
    }

    getApp().globalData.loginRetryCount = getApp().globalData.loginRetryCount+1;
